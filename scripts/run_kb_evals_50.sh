#!/bin/bash
# 50-node robustness sweep: 21 test sets x {MP-VNE, CARL-VNE CF e195}.
# Mirrors run_kb_evals.sh but on the 50-node substrate + 50-node checkpoint.
# Max 3 concurrent. Skips a job whose metrics.json already exists (resumable).
set -u
cd "$(dirname "$0")/.."

CKPT=experiments/carl_vne_50nodes/costfocused/checkpoints/il_mp_vne_v19_50nodes_cf_cont4_e45.pt
...
LOGDIR=experiments/carl_vne_50nodes/kb_eval_logs
mkdir -p "$LOGDIR"
MAXJOBS=3

run_eval() {
  local set=$1 algo=$2 name=$3 ckptflag=$4
  local sub=datasets/scenario_50nodes_${set}/substrate.json
  local req=datasets/scenario_50nodes_${set}/virtual_requests.json
  local out=results/scenario_50nodes_${set}/${name}
...
for set in $SETS; do
  ...
  run_eval "$set" carl_vne_pso carl_vne_cf_e195 "--checkpoint $CKPT" &
  while [ "$(jobs -r | wc -l)" -ge "$MAXJOBS" ]; do sleep 10; done
done
wait
echo "ALL 50-NODE KB EVALS DONE"
