"""50-epoch cost-focused PPO learning curves (zoomed reward + 6-panel grid).

Reads:
  logs/ppo_v19_50nodes_costfocused_epoch_summary.csv  (cf ep 1-10)
  logs/ppo_v19_50nodes_cf_cont_epoch_summary.csv      (cf ep 11-50, offset +10)

Reward = +0.5 + 1.0·rel_cost (success) | -0.5 (fail), per-VNR averaged.
"""
import csv
...
PHASES = [
    ("cf ep 1-10 (ref=V19 best)",
     "logs/ppo_v19_50nodes_costfocused_epoch_summary.csv", 0, "o", "tab:blue"),
    ("cf_cont ep 11-50 (ref=cf ep10)",
     "logs/ppo_v19_50nodes_cf_cont_epoch_summary.csv", 10, "s", "tab:cyan"),
    ("cf_cont2 ep 51-100 (ref=cf ep50)",
     "logs/ppo_v19_50nodes_cf_cont2_epoch_summary.csv", 50, "^", "tab:purple"),
]
...
def main():
    ...
    def plot_phases(ax, key, *, scale=1):
        ...
        for label, d, mk, col in phases:
            ax.plot(d["epoch"], d[key] * scale, mk, color=col, markersize=8,
    ...
    ax.scatter([all_epochs[peak]], [all_succ[peak]], s=250, marker="*",
               ...
               label=f"peak ep{int(all_epochs[peak])}: {all_succ[peak]:.2f}%")
    ...
    ax.set_xlabel("Epoch (cost-focused)"); ax.set_ylabel("Success rate (%)")
    ...
    ax.scatter([all_epochs[rpeak]], [all_reward[rpeak]], s=250, marker="*",
               color="gold", edgecolors="black", linewidths=1.2, zorder=10,
               label=f"peak ep{int(all_epochs[rpeak])}: {all_reward[rpeak]:+.4f}")
    ...
    ax.set_title("Mean reward per epoch (zoomed)",
                 fontsize=12, fontweight="bold")
    ax.set_xlabel("Epoch (cost-focused)"); ax.set_ylabel("Mean reward")
    ax.grid(alpha=0.3); ax.legend(fontsize=8, loc="lower right")
    ...
    ax.set_title("Mean total loss per epoch", fontsize=12, fontweight="bold")
    ax.set_xlabel("Epoch (cost-focused)"); ax.set_ylabel("Mean loss")
    ...
    ax.set_xlabel("Epoch (cost-focused)"); ax.set_ylabel("Mean policy_loss")
    ax.grid(alpha=0.3); ax.legend(fontsize=8)
    ...
    ax.set_title("Mean KL(π‖π_ref) per epoch",
                 fontsize=12, fontweight="bold")
    ax.set_xlabel("Epoch (cost-focused)"); ax.set_ylabel("Mean KL")
    ax.grid(alpha=0.3); ax.legend(fontsize=8)
    ...
    ax.set_title("Mean entropy per epoch (cand sharpening)",
                 fontsize=12, fontweight="bold")
    ax.set_xlabel("Epoch (cost-focused)"); ax.set_ylabel("Mean entropy")
    ax.grid(alpha=0.3); ax.legend(fontsize=8)

    fig.suptitle(
        f"V19 cost-focused PPO on 50-node — {int(all_epochs[-1])} epochs\n"
        f"Reward = +0.5 + 1.0·rel_cost (success) | −0.5 (fail)",
        fontsize=13, fontweight="bold")
    fig.tight_layout()
    out = OUT / f"50nodes_v19_cf_{int(all_epochs[-1])}ep_curves_{DATE_TAG}.png"
    ...
    ax.text(0.02, 0.96,
            f"cf ep1:  {all_reward[0]:+.4f}\n"
            f"cf ep{int(all_epochs[-1])}: {all_reward[-1]:+.4f}\n"
            f"peak ep{int(all_epochs[rpeak])}: {all_reward[rpeak]:+.4f}\n"
            f"Δ ep1 → ep{int(all_epochs[-1])}: {delta:+.4f}  ({'↑' if delta>0 else '↓'})",
            transform=ax.transAxes, fontsize=11, va="top", fontweight="bold",
            color="darkblue",
            bbox=dict(boxstyle="round", facecolor="#e3f2fd", alpha=0.92))
    pad = (all_reward.max() - all_reward.min()) * 0.25
    ax.set_ylim(all_reward.min() - pad, all_reward.max() + pad)
    ax.set_xlabel("Epoch (cost-focused)", fontsize=12)
    ax.set_ylabel("Mean reward per epoch", fontsize=12)
    ax.set_title(
        f"V19 cost-focused PPO on 50-node — reward per epoch ({int(all_epochs[-1])} epochs, zoomed)\n"
        f"Reward = +0.5 + 1.0·rel_cost (success) | −0.5 (fail)",
        fontsize=13, fontweight="bold")
    ax.grid(alpha=0.3)
    ax.legend(loc="lower right", fontsize=10)
    step = 2 if int(all_epochs[-1]) > 30 else 1
    ax.set_xticks(np.arange(1, int(all_epochs[-1]) + 1, step))
    fig.tight_layout()
    out2 = OUT / f"50nodes_v19_cf_{int(all_epochs[-1])}ep_reward_zoom_{DATE_TAG}.png"
