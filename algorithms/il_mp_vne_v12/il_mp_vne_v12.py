...
from algorithms.il_mp_vne_v12.policy_network import PolicyNetwork
from algorithms.il_mp_vne_v12.trainer import RankingTrainer
from algorithms.il_mp_vne_v12.vn_generator import generate_random_vn
...
class ILMPVNEV12:
    ...
    def __init__(self):
        ...
        self.policy = PolicyNetwork(
            ...
            vlink_feat_size=5,
            gcn_node_feat_size=11,      # V12: V6 (7) + max_bw, min_price, avg_delay, std_price
        ...
        if ckpt_path and os.path.exists(ckpt_path):
            ...
            print(f"  [IL-MP-VNE-V12] Loaded pretrained checkpoint: {ckpt_path}")
            if missing:
    ...
    def _extract_domain_features(self, lc: LocalController) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns: X (num_nodes, 11), A_norm (num_nodes, num_nodes)

        V12 adds (vs V6):
          [7] max(neighbor_bw_avail)   — strongest outgoing link bw
          [8] min(neighbor_bw_price)   — cheapest outgoing link
          [9] avg(neighbor_link_delay) — link delay neighborhood
         [10] std(neighbor_bw_price)   — price variability
        These give the model per-slink stats that were lost in V6's avg-only aggregation.
        """
        ...
        # V12: collect per-node lists for max/min/std/avg over individual links
        per_node_link_bw: List[List[float]] = [[] for _ in range(n)]
        per_node_link_price: List[List[float]] = [[] for _ in range(n)]
        per_node_link_delay: List[List[float]] = [[] for _ in range(n)]

        for (u, v), link in net.links.items():
            avail_bw = getattr(link, 'available_bw', link.bandwidth_capacity)
            bp = link.bandwidth_price
            ld = getattr(link, 'transmission_delay', 0.0)
            if u in id_to_idx:
                i = id_to_idx[u]
                degrees[i] += 1
                neighbor_bw[i] += avail_bw
                neighbor_bw_price[i] += bp
                per_node_link_bw[i].append(avail_bw)
                per_node_link_price[i].append(bp)
                per_node_link_delay[i].append(ld)
            if v in id_to_idx:
                i = id_to_idx[v]
                degrees[i] += 1
                neighbor_bw[i] += avail_bw
        ...
        # V12: per-link absolute scales (use individual link max for max_bw col)
        all_link_bws = [b for sub in per_node_link_bw for b in sub]
        all_link_prices = [p for sub in per_node_link_price for p in sub]
        all_link_delays = [d for sub in per_node_link_delay for d in sub]
        link_max_bw = max(all_link_bws) if all_link_bws else 1.0
        link_max_price = max(all_link_prices) if all_link_prices else 1.0
        link_max_delay = max(all_link_delays) if all_link_delays else 1.0
        ...
        for i, nid in enumerate(node_ids):
            ...
            # V12: per-slink statistics
            bws = per_node_link_bw[i]
            prs = per_node_link_price[i]
            dls = per_node_link_delay[i]
            if bws:
                X[i, 7] = max(bws) / link_max_bw                 # best outgoing capacity
                X[i, 8] = min(prs) / link_max_price              # cheapest outgoing
                X[i, 9] = (sum(dls) / len(dls)) / link_max_delay # avg link delay
                if len(prs) > 1:
                    mean_p = sum(prs) / len(prs)
                    var_p = sum((p - mean_p) ** 2 for p in prs) / (len(prs) - 1)
                    X[i, 10] = (var_p ** 0.5) / link_max_price   # price std
                else:
                    X[i, 10] = 0.0
            # else: X[i,7..10] = 0 (isolated node — leave default)
    ...
    def _pretrain(self, substrate_network: SubstrateNetwork) -> None:
        ...
        print(f"  [IL-MP-VNE-V12] Pre-training on {episodes} synthetic VNs...")
        self.policy.train()
        ...
        self.global_controller.clear_caches()
        self._pretrained = True
        print(f"  [IL-MP-VNE-V12] Pre-training complete.")
