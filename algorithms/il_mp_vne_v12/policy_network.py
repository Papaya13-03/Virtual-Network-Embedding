...
class JointCandidateHead(nn.Module):
    ...
    def __init__(self, vnode_dim: int, snode_dim: int, hidden_size: int = 64,
                 num_heads: int = 4, aux_dim: int = 4):
        super().__init__()
        assert hidden_size % num_heads == 0
        self.hidden_size = hidden_size
        self.num_heads = num_heads
        self.head_dim = hidden_size // num_heads
        self.aux_dim = aux_dim   # V6: [slack, cpu_cost, is_boundary, link_cost]
        ...
        # PreCost-equivalent explicit priors: NN starts with mp_vne-like
        # ranking and learns deviations.
        # V6: full PreCost ≈ α·cpu_cost + β·link_cost + boundary_offset
        self.cost_bias = nn.Parameter(torch.tensor(-3.0))         # cpu_cost down-weight
        self.boundary_bias = nn.Parameter(torch.tensor(0.0))
        self.link_cost_bias = nn.Parameter(torch.tensor(-3.0))    # V6: link_cost down-weight
    ...
    def forward(
        ...
        # V6: full PreCost-equivalent explicit prior
        explicit_prior = (
            self.cost_bias * cost_features[:, 0]
            + self.boundary_bias * cost_features[:, 1]
            + self.link_cost_bias * cost_features[:, 2]   # V6: link_term
        )
        ...
        if per_vnode_cpu_slacks is not None:
            ...
            for i in range(num_vnodes):
                ...
                cost_feat = (per_vnode_cost_features[i]
                             ...
                             else torch.zeros(pool.shape[0], 3))
