...
class GATLayer(nn.Module):
    """Single GAT (Graph Attention) layer with multi-head attention.

    For each node i and each neighbor j (where A[i,j] > 0), compute:
        e_ij = LeakyReLU(a^T [W·x_i || W·x_j])
        α_ij = softmax_j(e_ij)
        x_i' = Σ_j α_ij · W·x_j

    Multi-head: split out_dim into num_heads parallel heads, average.
    Memory: O(N² × heads) — fine for substrate (≤100 nodes).
    """
    ...
    def __init__(self, in_dim: int, out_dim: int, num_heads: int = 4,
        ...
        # Attention vectors per head: a_src and a_dst (decomposed for efficiency)
    ...
    def forward(self, X: torch.Tensor, A: torch.Tensor) -> torch.Tensor:
        # X: (N, in_dim), A: (N, N) — adjacency (treat any >0 as edge).
        n = X.shape[0]
        Wh = self.W(X).view(n, self.num_heads, self.head_dim)             # (N, H, D)
        # α_ij ∝ exp(a_src · Wh_i + a_dst · Wh_j)  [decomposed attention]
        e_src = (Wh * self.a_src.unsqueeze(0)).sum(dim=-1)                # (N, H)
        e_dst = (Wh * self.a_dst.unsqueeze(0)).sum(dim=-1)                # (N, H)
        # e_ij = e_src[i] + e_dst[j]
        e = e_src.unsqueeze(1) + e_dst.unsqueeze(0)                       # (N, N, H)
        e = self.leaky_relu(e)
        # Mask non-edges (A == 0) with -inf — softmax → 0 attention weight
        mask = (A.unsqueeze(-1) > 0).expand(-1, -1, self.num_heads)       # (N, N, H)
        e = e.masked_fill(~mask, float('-inf'))
        alpha = torch.softmax(e, dim=1)                                   # softmax over j (neighbors of i)
        # Replace NaNs (rows with no edge) with 0
        alpha = torch.nan_to_num(alpha, nan=0.0)
        # Aggregate: x_i' = Σ_j α_ij Wh_j
        out = torch.einsum("ijh,jhd->ihd", alpha, Wh)                     # (N, H, D)
        return out.reshape(n, -1)                                         # (N, out_dim)


class GATEncoder(nn.Module):
    """Multi-layer GAT with LayerNorm + residual."""

    def __init__(self, in_dim: int, hidden_size: int = 32,
        ...
        for i in range(num_layers):
            ...
            self.layers.append(GATLayer(d_in, hidden_size, num_heads=num_heads))
            self.norms.append(nn.LayerNorm(hidden_size))

    def forward(self, X: torch.Tensor, A: torch.Tensor) -> torch.Tensor:
        H = X
        for i, (gat, ln) in enumerate(zip(self.layers, self.norms)):
            H_new = torch.relu(gat(H, A))
            H = ln(H + H_new) if i > 0 else ln(H_new)
        return H
...
class HierarchicalSubstrateEncoder(nn.Module):
    ...
    def __init__(self, node_feat_size: int = 5,
        ...
        super().__init__()
        # V9: GAT for intra-domain (edge attention learns which neighbors matter).
        self.intra_gcn = GATEncoder(node_feat_size, intra_hidden,
                                    num_layers=intra_layers, num_heads=4)
...
class JointCandidateHead(nn.Module):
    ...
    def __init__(self, vnode_dim: int, snode_dim: int, hidden_size: int = 64,
                 num_heads: int = 4, aux_dim: int = 5):
        super().__init__()
        assert hidden_size % num_heads == 0
        self.hidden_size = hidden_size
        self.num_heads = num_heads
        self.head_dim = hidden_size // num_heads
        # V9 aux: [slack, cpu_cost, is_boundary, link_cost, slack_ratio]
        ...
        # V9 priors: calibrated less aggressive than V6 (-3.0 → -2.0) so policy
        # doesn't over-prefer marginal-feasibility cheap snodes. Added slack
        # bonus to actively prefer "safer" (higher-headroom) snodes.
        self.cost_bias = nn.Parameter(torch.tensor(-2.0))
        self.boundary_bias = nn.Parameter(torch.tensor(0.0))
        self.link_cost_bias = nn.Parameter(torch.tensor(-2.0))
        self.slack_ratio_bias = nn.Parameter(torch.tensor(1.5))   # V9: prefer high-slack
    ...
    def forward(
        ...
        cost_features: torch.Tensor,  # V6 (N, 3) — [cpu_cost, is_boundary, link_cost]
    ) -> torch.Tensor:
        n = snode_embeds.shape[0]
        denom = cpu_slack.abs().max().clamp(min=1.0)
        slack_norm = (cpu_slack / denom).unsqueeze(-1)

        aux = torch.cat([slack_norm, cost_features], dim=-1)            # (N, 4)
        ...
        # V9: PreCost-equivalent + explicit slack bonus
        explicit_prior = (
            self.cost_bias * cost_features[:, 0]
            + self.boundary_bias * cost_features[:, 1]
            + self.link_cost_bias * cost_features[:, 2]
            + self.slack_ratio_bias * cost_features[:, 3]   # V9: prefer high-slack
        )
        ...
        if per_vnode_cpu_slacks is not None:
            ...
            for i in range(num_vnodes):
                ...
                cost_feat = (per_vnode_cost_features[i]
                             ...
                             else torch.zeros(pool.shape[0], 4))
