# algorithms/il_mp_vne_v9/il_mp_vne_v9.py
import os
import random
import yaml
import torch
...
from algorithms.il_mp_vne_v9.policy_network import PolicyNetwork
from algorithms.il_mp_vne_v9.trainer import RankingTrainer
from algorithms.il_mp_vne_v9.vn_generator import generate_random_vn
...
class ILMPVNEV9:
    ...
    def __init__(self):
        ...
        self.policy = PolicyNetwork(
            ...
            vlink_feat_size=5,
            gcn_node_feat_size=9,       # V9: +domain_cpu_avail, +domain_bw_avail
        ...
        # Order: training.checkpoint (config) > default checkpoints/il_mp_vne_v9_pretrain.pt
        ckpt_path = self.config.get("training", {}).get("checkpoint")
        if not ckpt_path:
            default_ckpt = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                "checkpoints", "il_mp_vne_v9_pretrain.pt",
            )
        ...
        if ckpt_path and os.path.exists(ckpt_path):
            ...
            print(f"  [IL-MP-VNE-V9] Loaded pretrained checkpoint: {ckpt_path}")
            if missing:
    ...
    def _extract_domain_features(self, lc: LocalController) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Extract node features + adjacency.
        Returns: X (num_nodes, 9), A_norm (num_nodes, num_nodes)

        V9 adds: domain_cpu_availability_ratio, domain_bw_availability_ratio.
        """
        ...
        # Node features (V9): [avail_cpu, cpu_price, proc_delay, degree,
        #   avg_nb_bw, avg_nb_bw_price, is_boundary,
        #   domain_cpu_avail_ratio, domain_bw_avail_ratio]
        X = torch.zeros(n, 9)
        degrees = [0] * n
        neighbor_bw = [0.0] * n
        neighbor_bw_price = [0.0] * n

        # V9: domain-level availability (same value for all nodes in this domain)
        d_cpu_free = sum(getattr(nd, 'available_cpu', nd.cpu_capacity)
                         for nd in net.nodes.values())
        d_cpu_total = sum(nd.cpu_capacity for nd in net.nodes.values()) or 1.0
        d_bw_free = sum(getattr(l, 'available_bw', l.bandwidth_capacity)
                        for l in net.links.values())
        d_bw_total = sum(l.bandwidth_capacity for l in net.links.values()) or 1.0
        dom_cpu_ratio = d_cpu_free / d_cpu_total
        dom_bw_ratio = d_bw_free / d_bw_total
        ...
        for i, nid in enumerate(node_ids):
            ...
            # V9: domain-level availability (broadcast same value)
            X[i, 7] = dom_cpu_ratio
            X[i, 8] = dom_bw_ratio
        ...
        for vnode in vn.nodes.values():
            ...
            for idx in allowed_idx:
                for snode in all_node_lists[idx]:
                    ...
                    # V9: cpu_slack_ratio = (avail - demand) / capacity ∈ [-1, 1]
                    # Higher = "safer" (more headroom), lower = "tight" (might fail routing).
                    slack_ratio = (avail - vnode.cpu_demand) / max(snode.cpu_capacity, 1e-8)
                    slack_ratio = max(-1.0, min(1.0, slack_ratio))   # clamp

                    cost_feats.append((cpu_cost, is_boundary, link_cost_norm, slack_ratio))
    ...
    def _pretrain(self, substrate_network: SubstrateNetwork) -> None:
        ...
        print(f"  [IL-MP-VNE-V9] Pre-training on {episodes} synthetic VNs...")
        self.policy.train()
        ...
        self.global_controller.clear_caches()
        self._pretrained = True
        print(f"  [IL-MP-VNE-V9] Pre-training complete.")
