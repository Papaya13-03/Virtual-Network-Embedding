...
def generate_random_vn(
    ...
    allowed_domains_prob: float = 0.6,
    max_allowed_domains: int = 2,
) -> VirtualNetwork:
    """Generate a random connected virtual network.

    If `domain_ids` is provided, each vnode has `allowed_domains_prob` chance
    of getting a regional constraint (1..min(max_allowed_domains, len(domain_ids))
    domains sampled). This matches the eval distribution where ~60% of vnodes
    are domain-constrained. Without this, training would have a 100% "any
    domain" prior that the eval distribution doesn't share — train/eval mismatch.
    """
    ...
    for nid in node_ids:
        allowed = []
        if domain_ids and random.random() < allowed_domains_prob:
            k = random.randint(1, min(max_allowed_domains, len(domain_ids)))
            allowed = random.sample(domain_ids, k)
