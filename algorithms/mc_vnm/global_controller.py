...
class GlobalController:
    ...
    def commit_mapping(self, mapping: Dict[str, str], request: VirtualNetwork) -> Dict[tuple, List[SubstrateLink]]:
        """
        Commit resources using SINGLE-PATH mapping for MC-VNM.
        Returns a dictionary mapping virtual link keys to physical paths.
        """
        ...
        try:
            ...
            for vlink_key, vlink in request.links.items():
                ...
                # Find path using Kruskal's MST approach
                path = self.kruskal_path(src_snode, dst_snode, bw_required=vlink.bandwidth_demand)
    ...
    def kruskal_path(self, src: SubstrateNode, dst: SubstrateNode, bw_required: float = 0.0) -> List[SubstrateLink]:
        """Find a path using Kruskal's MST approach."""
        if src.id == dst.id:
            return []

        # 1. Gather all valid links
        valid_links = []
        all_nodes = set()
        ...
        # Sort links by cost (delay + price)
        valid_links.sort(key=lambda l: l.transmission_delay + l.bandwidth_price * bw_required)
        
        tree_links = []
        for link in valid_links:
            if union(link.source, link.target):
                tree_links.append(link)
            if find(src.id) == find(dst.id):
                break

        # 3. BFS search in the MST for the path
        if find(src.id) != find(dst.id):
            return []
        ...
        while queue:
            ...
            if curr == dst.id:
                return path
            for link in adj.get(curr, []):
                neighbor = link.target if link.source == curr else link.source
