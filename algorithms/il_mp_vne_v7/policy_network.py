...
class JointCandidateHead(nn.Module):
    ...
    def __init__(self, vnode_dim: int, snode_dim: int, hidden_size: int = 64,
        ...
        # V7 aux: [slack, cpu_cost, is_boundary, link_cost, util_after]
        ...
        # Explicit priors. V7 adds util_after_bias — pushes policy away from
        # near-saturating placements (forward-looking signal mp_vne lacks).
        ...
        self.util_after_bias = nn.Parameter(torch.tensor(-1.0))   # V7
    ...
    def forward(
        ...
        # V7: full PreCost-equivalent + forward-looking utilization
        explicit_prior = (
            self.cost_bias * cost_features[:, 0]
            + self.boundary_bias * cost_features[:, 1]
            + self.link_cost_bias * cost_features[:, 2]
            + self.util_after_bias * cost_features[:, 3]   # V7: util_after penalty
        )
...
class PolicyNetwork(nn.Module):
    ...
    def __init__(
        ...
        # V7: project global state (3 dims) → vn_hidden, ADD to vnode_embeds.
        # Every vnode gets the same global context injected (load-aware policy).
        self.global_state_proj = nn.Sequential(
            nn.Linear(3, vn_hidden),
            nn.LayerNorm(vn_hidden),
            nn.ReLU(),
            nn.Linear(vn_hidden, vn_hidden),
        )
    ...
    def forward(
        ...
        global_state: Optional[torch.Tensor] = None,                   # V7: (3,)
        ...
        # V7: inject global state context to every vnode embedding.
        if global_state is not None:
            gs_proj = self.global_state_proj(global_state)             # (vn_hidden,)
            vn_embeds = vn_embeds + gs_proj.unsqueeze(0)               # broadcast add
