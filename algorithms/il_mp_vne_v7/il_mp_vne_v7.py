# algorithms/il_mp_vne_v7/il_mp_vne_v7.py
import os
import random
import yaml
import torch
...
from algorithms.il_mp_vne_v7.policy_network import PolicyNetwork
from algorithms.il_mp_vne_v7.trainer import RankingTrainer
from algorithms.il_mp_vne_v7.vn_generator import generate_random_vn
...
class ILMPVNEV7:
    ...
    def __init__(self):
        ...
        # Order: training.checkpoint (config) > default checkpoints/il_mp_vne_v7_pretrain.pt
        ckpt_path = self.config.get("training", {}).get("checkpoint")
        if not ckpt_path:
            default_ckpt = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                "checkpoints", "il_mp_vne_v7_pretrain.pt",
            )
        ...
        if ckpt_path and os.path.exists(ckpt_path):
            ...
            print(f"  [IL-MP-VNE-V7] Loaded pretrained checkpoint: {ckpt_path}")
            if missing:
    ...
    def _extract_global_state(self) -> torch.Tensor:
        """V7: compute global substrate state — 3 numbers describing
        current load. Broadcast to every vnode, giving forward-looking
        context that mp_vne (greedy per-VNR) cannot use.

        Features:
          [0] total_cpu_utilization     Σ used_cpu / Σ capacity
          [1] total_bw_utilization      Σ used_bw / Σ capacity
          [2] domain_load_imbalance     std of per-domain cpu_util
        """
        gc = self.global_controller
        used_cpu_total, cap_cpu_total = 0.0, 0.0
        used_bw_total, cap_bw_total = 0.0, 0.0
        per_domain_util: List[float] = []
        for lc in gc.local_controllers:
            d_used, d_cap = 0.0, 0.0
            for node in lc.domain.network.nodes.values():
                avail = getattr(node, "available_cpu", node.cpu_capacity)
                d_used += node.cpu_capacity - avail
                d_cap += node.cpu_capacity
            for link in lc.domain.network.links.values():
                avail = getattr(link, "available_bw", link.bandwidth_capacity)
                used_bw_total += link.bandwidth_capacity - avail
                cap_bw_total += link.bandwidth_capacity
            used_cpu_total += d_used
            cap_cpu_total += d_cap
            per_domain_util.append(d_used / d_cap if d_cap > 0 else 0.0)
        for link in gc.snetwork.inter_domain_links.values():
            avail = getattr(link, "available_bw", link.bandwidth_capacity)
            used_bw_total += link.bandwidth_capacity - avail
            cap_bw_total += link.bandwidth_capacity

        cpu_util = used_cpu_total / cap_cpu_total if cap_cpu_total > 0 else 0.0
        bw_util = used_bw_total / cap_bw_total if cap_bw_total > 0 else 0.0
        if len(per_domain_util) > 1:
            mean_u = sum(per_domain_util) / len(per_domain_util)
            imbalance = (sum((u - mean_u) ** 2 for u in per_domain_util)
                         / len(per_domain_util)) ** 0.5
        else:
            imbalance = 0.0
        return torch.tensor([cpu_util, bw_util, imbalance], dtype=torch.float32)
    ...
    def _get_domain_inputs(
        ...
        for vnode in vn.nodes.values():
            ...
            slacks: List[float] = []
            cost_feats: List[Tuple[float, float, float, float]] = []
            for idx in allowed_idx:
                for snode in all_node_lists[idx]:
                    ...
                    # V7: forward-looking — what would utilization be AFTER mapping here?
                    # This is information mp_vne (greedy heuristic) doesn't use.
                    used_cpu = snode.cpu_capacity - avail
                    util_after = (used_cpu + vnode.cpu_demand) / max(snode.cpu_capacity, 1e-8)
                    util_after = min(util_after, 1.0)  # clamp to [0, 1]
                    cost_feats.append((cpu_cost, is_boundary, link_cost_norm, util_after))
    ...
    def _forward_policy(
        ...
        global_state = self._extract_global_state()    # V7: substrate-wide context
    ...
    def _pretrain(self, substrate_network: SubstrateNetwork) -> None:
        ...
        print(f"  [IL-MP-VNE-V7] Pre-training on {episodes} synthetic VNs...")
        self.policy.train()
        ...
        self.global_controller.clear_caches()
        self._pretrained = True
        print(f"  [IL-MP-VNE-V7] Pre-training complete.")
