# algorithms/il_mp_vne_v5/il_mp_vne_v5.py
import os
import random
import yaml
import torch
...
from algorithms.il_mp_vne_v5.policy_network import PolicyNetwork
from algorithms.il_mp_vne_v5.trainer import RankingTrainer
from algorithms.il_mp_vne_v5.vn_generator import generate_random_vn
...
class ILMPVNEV5:
    ...
    def __init__(self):
        ...
        self.policy = PolicyNetwork(
            vnode_feat_size=6,          # V5: +1 allowed_domains fraction
            vlink_feat_size=5,
        ...
        # Order: training.checkpoint (config) > default checkpoints/il_mp_vne_v5_pretrain.pt
        ckpt_path = self.config.get("training", {}).get("checkpoint")
        if not ckpt_path:
            default_ckpt = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                "checkpoints", "il_mp_vne_v5_pretrain.pt",
            )
        ...
        if ckpt_path and os.path.exists(ckpt_path):
            ...
            print(f"  [IL-MP-VNE-V5] Loaded pretrained checkpoint: {ckpt_path}")
            if missing:
    ...
    def _extract_vnode_features(self, vn: VirtualNetwork) -> torch.Tensor:
        ...
        Returns: (num_vnodes, 6) tensor.
        V5 adds: normalized count of allowed_domains (constraint strictness).
        """
        ...
        for i, nd in enumerate(vnodes):
            ...
            # V5: how constrained is this vnode's domain placement?
            # 1.0 = can go anywhere (unconstrained), small = highly constrained
    ...
    def _get_domain_inputs(
        ...
        # Normalize cpu_cost product by global max for stability
        # (max cpu_demand × max cpu_price = upper bound on node term)
        ...
        for vnode in vn.nodes.values():
            ...
            slacks: List[float] = []
            cost_feats: List[Tuple[float, float]] = []
            for idx in allowed_idx:
                for snode in all_node_lists[idx]:
                    ...
                    cost_feats.append((cpu_cost, is_boundary))
    ...
    def _pretrain(self, substrate_network: SubstrateNetwork) -> None:
        ...
        print(f"  [IL-MP-VNE-V5] Pre-training on {episodes} synthetic VNs...")
        self.policy.train()
        ...
        self.global_controller.clear_caches()
        self._pretrained = True
        print(f"  [IL-MP-VNE-V5] Pre-training complete.")
