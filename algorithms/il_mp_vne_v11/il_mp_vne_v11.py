...
from algorithms.il_mp_vne_v11.policy_network import PolicyNetwork
from algorithms.il_mp_vne_v11.trainer import RankingTrainer
from algorithms.il_mp_vne_v11.vn_generator import generate_random_vn
...
class ILMPVNEV11:
    ...
    def __init__(self):
        ...
        if ckpt_path and os.path.exists(ckpt_path):
            ...
            print(f"  [IL-MP-VNE-V11] Loaded pretrained checkpoint: {ckpt_path}")
            if missing:
    ...
    def _get_domain_inputs(
        ...
        # V11: pre-compute each vnode's PreCost-best snode (cheap O(N·|S|)).
        # Used to compute neighbor-conditional link cost — sharper signal than
        # averaging over neighbor's allowed_domains.
        vnode_best_dom: Dict[str, str] = {}
        for vnode_outer in vn.nodes.values():
            allowed_ids_o = vnode_outer.allowed_domains or domain_ids
            allowed_ids_o = [d for d in allowed_ids_o if d in dom_idx] or domain_ids
            best_cost = float("inf")
            best_dom = allowed_ids_o[0]
            for d in allowed_ids_o:
                for sn in all_node_lists[dom_idx[d]]:
                    av = getattr(sn, "available_cpu", sn.cpu_capacity)
                    if av < vnode_outer.cpu_demand:
                        continue
                    c = vnode_outer.cpu_demand * sn.cpu_price
                    if c < best_cost:
                        best_cost = c
                        best_dom = d
            vnode_best_dom[vnode_outer.id] = best_dom
        ...
        for vnode in vn.nodes.values():
            ...
            for idx in allowed_idx:
                for snode in all_node_lists[idx]:
                    ...
                    # V6 link cost: avg over neighbor's allowed_domains.
                    ...
                    for neighbor_vnode, bw in adj:
                        ...
                        # V11: neighbor-conditional link cost.
                        # Use neighbor's PreCost-best DOMAIN (committed guess).
                        nb_dom = vnode_best_dom.get(neighbor_vnode.id)
                        if snode_dom == nb_dom:
                            # same domain — intra link, ~0 cross-domain cost
                            nb_cross = 0.0
                        else:
                            nb_cross = cross_dom_price.get((snode_dom, nb_dom), 1.0)
                        nb_link_cost += bw * nb_cross

                    link_cost_norm = link_cost / max_link_cost
                    nb_link_cost_norm = nb_link_cost / max_link_cost
                    cost_feats.append((cpu_cost, is_boundary, link_cost_norm, nb_link_cost_norm))
    ...
    def _pretrain(self, substrate_network: SubstrateNetwork) -> None:
        ...
        print(f"  [IL-MP-VNE-V11] Pre-training on {episodes} synthetic VNs...")
        self.policy.train()
        ...
        self.global_controller.clear_caches()
        self._pretrained = True
        print(f"  [IL-MP-VNE-V11] Pre-training complete.")
