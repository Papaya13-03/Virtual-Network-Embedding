...
class JointCandidateHead(nn.Module):
    ...
    def __init__(self, vnode_dim: int, snode_dim: int, hidden_size: int = 64,
        ...
        # V11: [slack, cpu_cost, is_boundary, link_cost, nb_link_cost]
        ...
        # V11: PreCost-equivalent priors + nb_link_cost prior.
        # nb_link_cost is the SHARP signal — penalize harder than avg link_cost.
        self.cost_bias = nn.Parameter(torch.tensor(-3.0))
        self.boundary_bias = nn.Parameter(torch.tensor(0.0))
        self.link_cost_bias = nn.Parameter(torch.tensor(-1.5))      # softened (less sharp)
        self.nb_link_cost_bias = nn.Parameter(torch.tensor(-3.0))   # new: neighbor-conditional
    ...
    def forward(
        ...
        cost_features: torch.Tensor,  # V11 (N, 4) — [cpu_cost, is_boundary, link_cost, nb_link_cost]
    ) -> torch.Tensor:
        n = snode_embeds.shape[0]
        denom = cpu_slack.abs().max().clamp(min=1.0)
        slack_norm = (cpu_slack / denom).unsqueeze(-1)

        aux = torch.cat([slack_norm, cost_features], dim=-1)            # (N, 5)
        ...
        explicit_prior = (
            ...
            + self.nb_link_cost_bias * cost_features[:, 3]   # V11: neighbor-conditional
        )
