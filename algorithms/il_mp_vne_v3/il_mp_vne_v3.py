# algorithms/il_mp_vne_v3/il_mp_vne_v3.py
import os
import random
import yaml
import torch
...
from algorithms.il_mp_vne_v3.policy_network import PolicyNetwork
from algorithms.il_mp_vne_v3.trainer import RankingTrainer
from algorithms.il_mp_vne_v3.vn_generator import generate_random_vn
...
class ILMPVNEV3:
    ...
    def __init__(self):
        ...
        # Order: training.checkpoint (config) > default checkpoints/il_mp_vne_v3_pretrain.pt
        ckpt_path = self.config.get("training", {}).get("checkpoint")
        if not ckpt_path:
            default_ckpt = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                "checkpoints", "il_mp_vne_v3_pretrain.pt",
            )
        ...
        if ckpt_path and os.path.exists(ckpt_path):
            ...
            print(f"  [IL-MP-VNE-V3] Loaded pretrained checkpoint: {ckpt_path}")
            if missing:
    ...
    def _get_domain_inputs(
    ...
    ) -> Tuple[list, list, list, list, list, list, torch.Tensor]:
        ...
        Returns (all_domain_feats, all_domain_adjs, all_domain_node_lists,
                 ...
                 per_vnode_slacks, inter_domain_adj).
        """
    ...
    def _forward_policy(
        ...
        (all_feats, all_adjs, _all_node_lists,
         per_vnode_allowed_idx, per_vnode_pools,
         per_vnode_slacks, inter_adj) = self._get_domain_inputs(vn)
    ...
    def _pretrain(self, substrate_network: SubstrateNetwork) -> None:
        ...
        print(f"  [IL-MP-VNE-V3] Pre-training on {episodes} synthetic VNs...")
        self.policy.train()
        ...
        self.global_controller.clear_caches()
        self._pretrained = True
        print(f"  [IL-MP-VNE-V3] Pre-training complete.")
