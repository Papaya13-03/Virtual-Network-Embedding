...
class JointCandidateHead(nn.Module):
    """Cross-attention scorer; snode context = [intra_embed, domain_embed]."""
    ...
    def __init__(self, vnode_dim: int, snode_dim: int, hidden_size: int = 64,
        ...
        self.key_proj = nn.Linear(snode_dim + 1, hidden_size, bias=True)  # +1 for slack
        self.residual = nn.Sequential(
            nn.Linear(vnode_dim + snode_dim + 1, hidden_size),
    ...
    def forward(
        ...
        snode_embeds: torch.Tensor,   # (N, snode_dim) — intra+inter concat
        ...
        if per_vnode_cpu_slacks is not None:
            ...
            for i in range(num_vnodes):
                ...
                if pool.shape[0] == 0:
                    ...
                    continue
                scores = self.candidate_head(vn_embeds[i], pool, slack)
