...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    font=\large,
    blk/.style={draw, rounded corners, minimum height=14mm, text width=32mm, align=center, thick},
    io/.style={blk, fill=white},
    enc/.style={blk, fill=blue!8},
    key/.style={blk, draw=orange!80!black, fill=orange!25, very thick},
    proc/.style={blk, fill=black!8},
    grp/.style={draw, dashed, rounded corners, inner sep=5mm, line width=0.6pt},
    >={Stealth[length=3.5mm,width=3mm]},
  ]
    % ----- (1) Bộ mã hoá đồ thị -----
    \node[io]  (gs)   at (0,2.0)    {Mạng vật lý $G^s=(N^s,E^s)$};
    \node[io]  (vnr)  at (0,-2.0)   {Yêu cầu mạng ảo $G^v=(N^v,E^v)$};
    \node[enc] (encs) at (5,2.0)    {Mã hoá phân cấp (GCN nội miền + liên miền)};
    \node[enc] (encv) at (5,-2.0)   {Mã hoá VNR (GCN + self-attention)};
    \node[enc] (fuse) at (10,0)     {Trộn biểu diễn (ngữ cảnh substrate--VNR)};

    % ----- (2) Hai đầu ra -----
    \node[key] (cand) at (15,2.0)   {Đầu chọn ứng viên (cross-attention)};
    \node[enc] (val)  at (15,-2.0)  {Bộ phê bình giá trị $V(s)$};

    % ----- (3) Bộ giải mã -----
    \node[proc] (dec) at (15,-7.0)  {Bộ giải mã: ánh xạ nút theo điểm ứng viên + ánh xạ liên kết đa đường};

    % ----- (4) Môi trường -----
    \node[proc] (env) at (7.5,-7.0) {Môi trường: cam kết tài nguyên + trả về thưởng $r$};

    % luồng dữ liệu
    \draw[->,thick] (gs)   -- (encs);
    \draw[->,thick] (vnr)  -- (encv);
    \draw[->,thick] (encs) -- (fuse);
    \draw[->,thick] (encv) -- (fuse);
    \draw[->,thick] (fuse) -- (cand);
    \draw[->,thick] (fuse) -- (val);
    \draw[->,thick] (cand.east) -- ++(0.7,0) |- (dec.east);
    \draw[->,thick] (dec) -- (env);
    % phản hồi thưởng (cập nhật toàn bộ các khối học)
    \draw[->,thick,dashed,red!70!black]
       (env) -- node[pos=0.5, fill=white, inner sep=2pt, font=\normalsize, text=black]{thưởng $r$,\ $V(s)$} (fuse);

    % khung nhóm khối
    \begin{scope}[on background layer]
      \node[grp, fit=(gs)(vnr)(encs)(encv)(fuse),
            label={[font=\large\bfseries]north:(1) Bộ mã hoá đồ thị}] {};
      \node[grp, fit=(cand)(val),
            label={[font=\large\bfseries]north:(2) Hai đầu ra}] {};
      \node[grp, fit=(dec),
            label={[font=\large\bfseries]north:(3) Bộ giải mã}] {};
      \node[grp, fit=(env),
            label={[font=\large\bfseries]north:(4) Môi trường}] {};
    \end{scope}
  \end{tikzpicture}%
  }
  \caption[Kiến trúc tổng thể của CARL-VNE.]{Kiến trúc tổng thể của CARL-VNE. Các khối tô màu (bộ mã hoá, đầu chọn ứng viên, bộ phê bình) được tinh chỉnh bằng học tăng cường; tín hiệu học được dồn vào đầu chọn ứng viên (khối tô cam).}
  \label{fig:arch}
\end{figure}
...
\section{Thiết kế môi trường học tăng cường}            % ~2.5 trang
\label{sec:env}
...
\section{Các giai đoạn huấn luyện}   % ~3.5 trang
\label{sec:training}
